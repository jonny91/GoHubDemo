...
func ShutdownService(url string) error {
	...
	if res.StatusCode != http.StatusOK {
		return fmt.Errorf("failed to deregister service. Registry service responded with code %d", res.StatusCode)
	}
	return nil
}
...
func (p *providers) get(name ServiceName) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("no provoders available for service %s", name)
	}
